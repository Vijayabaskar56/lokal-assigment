  ...
  return isLoading ?
    ...
    <View flex={1} marginHorizontal='$3' marginVertical='$3'>
      <YStack marginVertical='$3'>
        ...
        <ZStack top={0} left={-30} onPress={() =>  storage.set(`${job?.id}`, true)}>
      ...
      <XStack gap='$4' justifyContent="flex-start" alignItems="center" alignSelf="stretch" marginVertical='$3'>
        <YStack gap='$4'>
          <IconText content="Living Place" Icon={<MapPin size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job?.primary_details?.Place}</Text>
          </IconText>
          <IconText content="Experience" Icon={<Briefcase size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job?.primary_details?.Experience}</Text>
          </IconText>
          <IconText content="Job Hours" Icon={<Clock10 size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job?.job_hours}</Text>
          </IconText>
          <IconText content={job.contentV3.V3[1].field_key} Icon={<UserCircle2 size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job.contentV3.V3[1].field_value}</Text>
          </IconText>
        </YStack>
        <YStack gap='$4'>
          <IconText content="Salary" Icon={<Banknote size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job?.primary_details?.Salary}</Text>
          </IconText>
          <IconText content={job?.job_hours} Icon={<BookOpen size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job?.primary_details?.Qualification}</Text>
          </IconText>
          <IconText content={job.contentV3.V3[2].field_key} Icon={<Sheet size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job.contentV3.V3[2].field_value}</Text>
          </IconText>
          <IconText content="Vacancies" Icon={<Users size={20} />}>
            <Text fontSize='$6' wordWrap="break-word" >{job?.openings_count}</Text>
          </IconText>
        </YStack>
      </XStack>
      <Separator />
